...
object GetOutputsCircuit extends App {
    ...
    def getCEx(json: String, filename: String): String = {
        ...
        val transformedInputValues = inputValues.arr.map { inputValue =>
            println(inputValue("inputs"))
            val inputsBoolList = inputValue("inputs").arr.map { inputStr =>
            inputStr.str.toLowerCase match {
                case "true" => true
                case "false" => false
                case _ => throw new IllegalArgumentException(s"Unexpected input value: $inputStr")
            }}
            val inputsList = Iterable((query zip inputsBoolList).toMap)
        ...
        val directoryPath = "src/test/benchmarks/circuit/test/"
        val jsonFilePath = "completions/circuit-generated-examples.json"
        val outputFileName = "aggregate-results-circuit.json" // Name of the file where you want to save all results
    ...
    //getCEx("completions/circuit-generated-examples.json", "src/test/benchmarks/circuit/test/CrCy_10-sbox2-D7-sIn1.sl")
}